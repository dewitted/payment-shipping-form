...
const Select = ({ options, label }) => {
  ...
  const list = options.map((item, index) => {
    return (
      <div
        className="optionsContainer__options"
        key={index}
    ...
    <>
      <div className="wrapper">
        <div className="selectContainer" onClick={() => onClickHandler()}>
          <span className="selectContainer__label">
            <h3>{label}</h3>
          </span>
          <span
            className={
              isRotated
                ? "selectContainer__arrow rotated"
                : "selectContainer__arrow"
            }
          ></span>
          <h1>{selectValue}</h1>
        </div>
        <div
          className={isOpened ? "optionsContainer" : "optionsContainer hidden"}
        >
          {list}
        </div>
      </div>
